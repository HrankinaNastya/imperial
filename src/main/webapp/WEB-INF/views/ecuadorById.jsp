				...
				<table cellspacing="0" cellpadding="0" id="submain">
					<tr valign="top">
						<td width="30%" id="lft"><img
							...
							class="h1but">
							<div class="h1">
								<a href="<c:url value="/ecuador" />" class="roboto5"> <c:if
							...
							<div class="submenu">
								<c:if test="${!empty subcategoryList}">
									<c:forEach items="${subcategoryList}" var="subcategory">
										<li><a
											href="<c:url value="/ecuador/${subcategory.id}" />">${subcategory.name}</a></li>
									</c:forEach>
								</c:if>
						...
						<td class="lh" style="padding: 0 23px;">
							...
							<div style="position: relative; top: -230px; clear: both;">
								<h1 class="roboto1">
									<c:if test="${!empty subcategory}">${subcategory.name}</c:if>
								</h1>
								<p>
