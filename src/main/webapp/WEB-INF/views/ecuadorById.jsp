...
<title><spring:message code="label.imperial.ecuador" /></title>
				...
				<div class="menu">
					...
					<div>
						<img src="<c:url value="/resources/images/arrow.png" />"><a
							...
							href="<c:url value="/logistics${locale}" />" class="roboto4 lastrob"><c:if
				...
				<table cellspacing="0" cellpadding="0" id="submain">
					<tr valign="top">
						<td width="30%" id="lft"><img
							...
							class="h1but">
							<div class="h1">
								<a href="<c:url value="/ecuador${locale}" />" class="roboto5"><c:choose>
							...
							<div class="submenu">
								<c:if test="${!empty subcategoryList}">
									<c:forEach items="${subcategoryList}" var="subcategory">
										<li><a
											href="<c:url value="/ecuador/${subcategory.id}${locale}" />"><c:choose>
									...
									<c:if test="${!empty subcategory}">
										<c:choose>
											<c:when test="${lang eq 'en'}">
												<p>${subcategory.content_en}</p>
											</c:when>
											<c:when test="${lang eq 'es'}">
												<p>${subcategory.content_es}</p>
											</c:when>
											<c:otherwise>
